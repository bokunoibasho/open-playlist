...
> **リリース運用（〜Phase 8）**: ロードマップ期は **Phase N = `v0.N.0`** に対応させた。
> Phase 1〜8 はリリース済み（`v0.1.0`〜`v0.8.0`、間に複数のパッチ
> `v0.4.1` / `v0.7.1` / `v0.8.1`〜`v0.8.3`）。**Phase 8 でロードマップ上のフェーズは完了**し、
> Phase 9 以降は定義しない（以後の運用は §9 末尾「リリース運用（Phase 8 以降）」を参照）。
...
ロードマップ（フェーズ）を消化したため、以後は **Phase=minor を廃止し、通常の
セマンティックバージョニング**に移行する。

- **minor (`0.x.0`)**: ユーザーに見える新機能を 1 つ追加したとき。次のまとまった
  新機能が `v0.9.0`、以降 `0.10.0` / `0.11.0`… と続く（minor は 1 桁制限ではなく、
  `0.9` は `1.0` への「カウントダウン」ではない）。
- **patch (`0.x.y`)**: バグ修正・見た目調整・既存機能の改善（例: #46 のサムネ修正）。

#### 1.0.0 の線引き

このアプリの 1.0 は **ストア公開ではない**（§2 のとおり配布はスコープ外。YouTube ToS 懸念）。
1.0 は「**自分が"完成品"として日常使いできる**」という個人的マーカーとする。

> **1.0.0 の条件**: 実機で数週間「日常の音楽アプリ」として使い、ブロッカー級のバグが
> 出ない。かつ **GitHub の `v1.0.0` マイルストーン**に積んだ「1.0 必須」Issue が
> すべて閉じている。それ以外の Issue はすべて post-1.0（`1.1.0` 以降）。

コア体験（§1: Web の動画を「曲」として追加 → Apple Music 風にバックグラウンド/
ロック画面/PiP 再生 + オフライン保存）は Phase 6〜8 で機能として概ね揃っている。
1.0 を分けるのは機能の不足ではなく、仕上がりと実運用の安定性。
